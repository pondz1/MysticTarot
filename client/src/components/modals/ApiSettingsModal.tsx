...
import { Settings, X, Key, Globe, Cpu, Check, Sparkles, Coins, PlusCircle } from 'lucide-react';
...
export const ApiSettingsModal: React.FC<ApiSettingsModalProps> = ({
  ...
  const { credits: authCredits } = useAuth();
        ...
        <div className="relative w-full max-w-lg glass-panel-gold rounded-2xl p-6 border border-amber-400/50 shadow-2xl overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-2 mb-5">
            <button
              ...
              className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition-all text-xs font-semibold cursor-pointer ${mode === 'credit'
                ? 'bg-amber-500/20 border-amber-400 text-amber-300 shadow-md shadow-amber-500/10'
                : 'bg-purple-950/40 border-purple-500/20 text-purple-300 hover:bg-purple-900/40'
                }`}
            >
              <Coins className="w-4 h-4 text-amber-400" />
              <span>ระบบ Credit</span>
            </button>

            <button
              type="button"
              onClick={() => setMode('custom')}
              className={`flex items-center justify-center gap-2 p-3 rounded-xl border transition-all text-xs font-semibold cursor-pointer ${mode === 'custom'
                ? 'bg-amber-500/20 border-amber-400 text-amber-300 shadow-md shadow-amber-500/10'
                : 'bg-purple-950/40 border-purple-500/20 text-purple-300 hover:bg-purple-900/40'
                }`}
            >
            ...
            <div className="flex flex-col gap-4 mb-5 p-4 rounded-xl bg-purple-950/60 border border-amber-500/30">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-purple-950 shadow-md shadow-amber-500/20 transition-all cursor-pointer"
                >
                  <PlusCircle className="w-4 h-4" />
                  <span>เติม เครดิต (Top-Up Simulator)</span>
                </button>
              </div>

              <p className="text-[11px] text-purple-300 leading-relaxed border-t border-purple-800/40 pt-2.5">
                ✨ ใช้บริการ AI ผ่าน Server ของระบบโดยตรง ไม่ต้องมี API Key ของตัวเอง คิดค่าบริการตามจำนวน Token ที่ใช้งานจริง (สามารถคลิกปุ่ม <strong>เติม เครดิต</strong> เพื่อทดลองจำลองการเติมเงินได้)
              </p>
            </div>
          )}
